  ...
  function validateProductionRows(rows) {
    ...
    return rows.map((row, index) => {
      ...
      if (!Number.isNaN(quantity) && quantity < 0) {
        issues.push({ type: 'error', message: 'La cantidad es negativa.' });
      }

      const duplicateKey = `${String(row.fecha).trim().toLowerCase()}|${String(row.producto).trim().toLowerCase()}|${String(row.responsable).trim().toLowerCase()}`;
      if (!empty(row.fecha) && !empty(row.producto) && !empty(row.responsable)) {
        if (seen.has(duplicateKey)) {
          issues.push({ type: 'warning', message: 'Registro duplicado por fecha, producto y responsable.' });
        } else {
